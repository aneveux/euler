...
class Problem8 : Problem() {
    ...
    fun maxProductOfAdjacentDigits(digits: Long) = Stream.ofAll(grid.toList()).map(Char::toIntValue).sliding(digits).map(Stream<Int>::product).max().get()

    override fun solve() = maxProductOfAdjacentDigits(13L).toString()
}

class Problem9 : Problem() {
    override fun solve() = TODO()
}