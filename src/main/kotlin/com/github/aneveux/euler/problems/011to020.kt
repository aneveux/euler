...
class Problem11 : Problem() {
    ...
    private val grid = arrayOf(intArrayOf(8, 2, 22, 97, 38, 15, 0, 40, 0, 75, 4, 5, 7, 78, 52, 12, 50, 77, 91, 8),
    ...
    override fun solve() = Stream.range(0, 16).flatMap { x ->
        Stream.range(0, 16).flatMap { y ->
            Stream.of(
                    Stream.range(0, 4).map { grid[x + it][y] }.toList(),
                    Stream.range(0, 4).map { grid[x + it][y + it] }.toList(),
                    Stream.range(0, 4).map { grid[x + it][4 + y - it] }.toList(),
                    Stream.range(0, 4).map { grid[x][y + it] }.toList()
            )
        }
...
/**
 * Solving [https://projecteuler.net/problem=13]
 *
...
class Problem19 : Problem() {
    ...
    fun countFirstSundays2() = (1901..2000).asSequence()
            ...
            .filter {
                ...
                it.atDay(1).dayOfWeek.equals(Calendar.SUNDAY)
            }
            .count()
