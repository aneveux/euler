...
class Problem1 : Problem() {
    fun sumMultiples(limit: Int): Number = Stream.range(1, limit)
...
class Problem5 : Problem() {
    override fun solve() = Stream.from(19, 19).first { it.isEvenlyDivisibleBy((1..20)) }.toString()
}
...
class Problem8 : Problem() {
    ...
    fun maxProductOfAdjacentDigits(digits: Int): Number = Stream.ofAll(grid.toList())
            .map(Char::toIntValue).sliding(digits).map(Stream<Int>::product).max().get()
